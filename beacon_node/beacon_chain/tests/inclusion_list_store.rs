...
fn transaction(byte: u8) -> Transaction<<E as EthSpec>::MaxBytesPerTransaction> {
...
fn signed_inclusion_list(
    ...
    SignedInclusionList {
        message: InclusionList {
            ...
            transactions: VariableList::new(vec![transaction(tx_byte)]).unwrap(),
        },
...
/// Reads target `slot - 1`, which falls in the previous epoch once per epoch. Anchoring on a block
/// root from the slot's own epoch keeps that resolvable, where the head root does not.
#[tokio::test]
...
async fn bits_and_transactions_read_back_through_the_chain() {
    ...
    let committee = harness
        ...
        .inclusion_list_committee(block_root, slot)
        .unwrap();
    let committee_root = InclusionListStore::<E>::committee_root(&committee).unwrap();
    ...
    store.process_inclusion_list(
        signed_inclusion_list(slot, timely_submitter, committee_root, 0xaa),
        true,
    );
    store.process_inclusion_list(
        signed_inclusion_list(slot, late_submitter, committee_root, 0xbb),
        false,
    );
    drop(store);
...
async fn per_slot_task_prunes_the_store() {
    ...
    harness
        ...
        .process_inclusion_list(
            signed_inclusion_list(slot, committee[0], committee_root, 0xaa),
            true,
        );
